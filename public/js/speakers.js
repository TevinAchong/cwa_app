firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
        user.getIdToken().then(function(accessToken) {
            ...
            db.collection('speakers').orderBy('lastname').onSnapshot(snapshot => { // Listing all the files in ascending order according to file name
                ...
                changes.forEach(change => {
                    ...
                    if (change.type == 'added') {
                        ...
                        */
                        ...
                        card_title.innerHTML = change.doc.data().firstname + change.doc.data().lastname; 
                        card_title.classList.add('card-title'); 
                        card_content.appendChild(card_title);

                        let card_par = document.createElement('p'); 
                        card_par.innerHTML = change.doc.data().bio;
